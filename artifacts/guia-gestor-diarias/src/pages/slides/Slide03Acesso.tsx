export default function Slide03Acesso() {
  return (
    <div style={{ width: '100vw', height: '100vh', overflow: 'hidden', backgroundColor: '#FAFAFA', fontFamily: "'Inter', sans-serif", position: 'relative', boxSizing: 'border-box', padding: '5vh 5vw', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '5vh' }}>
        <div style={{ position: 'relative' }}>
          <div style={{ position: 'absolute', left: '-1vw', top: '1.5vh', width: '17vw', height: '2.5vh', backgroundColor: '#0A1628', opacity: 0.07, zIndex: 0 }} />
          <h2 style={{ fontSize: '3.5vw', fontWeight: 900, color: '#0A1628', margin: 0, lineHeight: 1, letterSpacing: '-0.03em', position: 'relative', zIndex: 1 }}>Acesso ao Sistema</h2>
        </div>
        <div style={{ fontSize: '1.2vw', fontWeight: 800, color: '#0A1628', letterSpacing: '-0.02em' }}>DECARGO</div>
      </div>

      {/* URL hero */}
      <div style={{ backgroundColor: '#0A1628', padding: '4vh 3.5vw', marginBottom: '5vh' }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '1vw', color: '#718096', marginBottom: '1.5vh', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Endereço de acesso</div>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '3.2vw', fontWeight: 600, color: '#FFFFFF', letterSpacing: '-0.01em' }}>
          app.decargopeople.com.br
        </div>
      </div>

      {/* Bullets */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '3vh' }}>
        <div style={{ display: 'flex', gap: '2vw', alignItems: 'flex-start' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '1.5vw', color: '#A0AEC0', fontWeight: 600, flexShrink: 0, width: '2.5vw' }}>01</div>
          <p style={{ fontSize: '2vw', color: '#4A5568', lineHeight: 1.45, margin: 0 }}>Faça login com sua conta institucional DECARGO</p>
        </div>
        <div style={{ width: '100%', height: '1px', backgroundColor: '#E2E8F0' }} />
        <div style={{ display: 'flex', gap: '2vw', alignItems: 'flex-start' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '1.5vw', color: '#A0AEC0', fontWeight: 600, flexShrink: 0, width: '2.5vw' }}>02</div>
          <p style={{ fontSize: '2vw', color: '#4A5568', lineHeight: 1.45, margin: 0 }}>Seu perfil de Gestor é definido automaticamente pelo sistema</p>
        </div>
        <div style={{ width: '100%', height: '1px', backgroundColor: '#E2E8F0' }} />
        <div style={{ display: 'flex', gap: '2vw', alignItems: 'flex-start' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '1.5vw', color: '#A0AEC0', fontWeight: 600, flexShrink: 0, width: '2.5vw' }}>03</div>
          <p style={{ fontSize: '2vw', color: '#4A5568', lineHeight: 1.45, margin: 0 }}>Em caso de problemas de acesso, contate o administrador</p>
        </div>
      </div>

      {/* Footer */}
      <div style={{ position: 'absolute', bottom: '3vh', left: '5vw', right: '5vw', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #E2E8F0', paddingTop: '2vh' }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.9vw', color: '#A0AEC0' }}>Guia do Gestor / DECARGO Diárias</div>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.9vw', color: '#0A1628', fontWeight: 600 }}>03</div>
      </div>
    </div>
  );
}
